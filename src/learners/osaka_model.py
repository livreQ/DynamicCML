...
# Created Time : Thu Dec 29 01:04:17 2022
# File Name: osaka_model.py
# Description:
"""
...
class MetaConvModel(MetaModule):
    ...
    def __init__(self, in_channels, out_features, hidden_size=64, feature_size=64, deeper=0):
        ...
        self.deeper = deeper
        self.config = [ ('conv2d', [hidden_size, in_channels, 3, 3, (1,1), (1,1)]), 
                        ('bn', [hidden_size, 1., False]),
                        ('relu', [False]),
                        ('max_pool2d', [2, 2, 0, 1]),
                        ('conv2d', [hidden_size, hidden_size, 3, 3, (1,1), (1,1)]), 
                        ('bn', [hidden_size, 1., False]),
                        ('relu', [False]),
                        ('max_pool2d', [2, 2, 0, 1]),
                        ('conv2d', [hidden_size, hidden_size, 3, 3, (1,1), (1,1)]), 
        ...
        if self.deeper:
            ...
            for i in range(self.deeper):
                ...
                self.config.append(('linear', [feature_size, feature_size]))

            self.features = MetaSequential(OrderedDict(self.features))
        self.classifier = MetaLinear(feature_size, out_features, bias=True)
        self.config.append(('flatten', []))
        self.config.append(('linear', [out_features, feature_size]))
    ...
    # def summary2dict(self):
    #     summary(self.model, (self.in_features))
...
class MetaMLPModel(MetaModule):
    ...
    def __init__(self, in_features, out_features, hidden_sizes):
        ...
        for (i, hidden_size) in enumerate(layer_sizes[:-1]):
            self.config += [('linear', [hidden_size, layer_sizes[i + 1]]),
                                ('relu', [True])]
        self.classifier = MetaLinear(hidden_sizes[-1], out_features, bias=True)
        self.config.append(('linear', [hidden_sizes[-1], out_features]))
